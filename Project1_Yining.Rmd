...
ggplot(data = df) +
 ...
 geom_point()
```
^ Children in high income groups tend to be less likely to experience childhood stunting.
...
```
^EURO has the lowest childhood stunting prevalence. EMRO and WPRO had bigger ranges than the rest regions. 



4. national prevalence & prevalence of mothers with no education
...
ggplot(data = df) +
  ...
  aes(color = who_regions)
```
^Besides EURO, the childhood stunting prevalence of other WHO regions is in positive correlation with the prevalence of mothers with no education.

5. national prevalence & male prevalence/female prevalence
